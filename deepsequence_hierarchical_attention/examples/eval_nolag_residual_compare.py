#!/usr/bin/env python3
"""
No-lag fair bake-off (windows replace engineered lags):

Feature contract (shared):
  trend + cyclical + holiday + intermittent
  DROPPED: lag_1 / lag_2 / lag_7  (covered by lookback windows)

Models:
  1. DeepSequence (no lags) + residual TF (preserve DS gate)
  2. DeepAR-lite (Quantity + no-lag causal features in lookback)
  3. Temporal transformer (same windows)
  4. LightGBM (same no-lag tabular features) — reference

All use three_term (NN) / L1 (LightGBM). Same 800 SKUs / seed=42.
"""
...
def parse_args():
    ...
    p.add_argument(
        "--out_json",
        default=str(ROOT / "eval_results_nolag_residual_compare.json"),
    )
    return p.parse_args()


def drop_lag_columns(X_df: pd.DataFrame, cfg) -> tuple[pd.DataFrame, list[str]]:
    """Keep intermittent; drop engineered lag_* columns only."""
    lag_set = set(cfg.lag_names)
    keep = [c for c in X_df.columns if c not in lag_set]
    dropped = [c for c in X_df.columns if c in lag_set]
    return X_df[keep].copy(), dropped


def component_indices_nolag(feature_names: list[str], cfg):
    """Map component slices onto the no-lag feature matrix column order."""
    name_to_i = {n: i for i, n in enumerate(feature_names)}

    def idx(names):
        return [name_to_i[n] for n in names if n in name_to_i]

    trend = idx(["time_index"])
    seasonal = idx(cfg.cyclical_names)
    # holidays = distance + binary holiday names in config order
    holiday = idx(cfg.holiday_names + cfg.binary_holiday_names)
    intermittent = idx(cfg.intermittent_names)
    return trend, seasonal, holiday, intermittent
...
def main():
    ...
    print("Building causal features, then dropping engineered lags only...")
    ...
    print(f"dropped lags={dropped}")
    print(f"kept features ({len(feature_names)}): {feature_names}")

    X_train = Xtr_df.to_numpy(np.float32)
    X_val = Xva_df.to_numpy(np.float32)
    X_test = Xte_df.to_numpy(np.float32)
    # normalize time_index if present
    ...
    assert inter_i, "intermittent features missing after lag drop"
    ...
    results = {
        "config": {
            ...
            "kept_intermittent": cfg.intermittent_names,
            "note": "Engineered lags removed; lookback windows cover short-term memory. Intermittent state kept.",
    ...
    # ------------------------------------------------------------------
    # DS no-lag (intermittent only in regressor) + residual preserve-gate
    # ------------------------------------------------------------------
    print("\n=== DS (no lags, keep intermittent) + residual TF ===")
    ...
    # structural-only metrics
    results["models"]["ds_nolag_structural_only"] = {
        "train_seconds": ds_s,
        "overall": kpi_block(
            y_test, np.asarray(pred_te["final_forecast"]).reshape(-1), p_te
        ),
    ...
    results["models"]["ds_nolag_residual_preserve_gate"] = {
        "train_seconds": ds_s + tf_s,
        "overall": kpi_block(yr[r_te], yhat_r, p_r),
        "strata": strata_report(yr[r_te], yhat_r, p_r, skur[r_te], volume_map),
        "note": "DS without engineered lags; residual TF lookback supplies short-term memory; p_ds preserved",
    }

    # ------------------------------------------------------------------
    # LightGBM no-lag
    # ------------------------------------------------------------------
    print("\n=== LightGBM (no engineered lags) ===")
    ...
    # ------------------------------------------------------------------
    # DeepAR + TST on no-lag full windows
    # ------------------------------------------------------------------
    print("\nBuilding no-lag sequence windows for DeepAR/TST...")
    ...
    print(f"sequence channels={n_channels} (Quantity + {n_channels-1} no-lag causal feats)")
    ...
    for name, builder in [
        ("deepar_lite_nolag", build_deepar),
        ("temporal_transformer_nolag", build_transformer),
    ]:
    ...
    print("\n" + "=" * 70)
    print("NO-LAG COMPARISON (windows replace engineered lags)")
    print("=" * 70)
